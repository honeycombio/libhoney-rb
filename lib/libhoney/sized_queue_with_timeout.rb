...
module Libhoney
  ...
  class SizedQueueWithTimeout
    ...
    # @param timeout [Numeric, :never] how long in seconds to wait for the queue to have space available or +:never+ to wait "forever"
    # @param timeout_policy [#call] defaults to +-> { raise PushTimedOut }+ - a lambda/Proc/callable, what to do when the timeout expires
    # @raise PushTimedOut
    ...
    #
    # @param timeout [Numeric, :never] how long in seconds to wait for the queue to have an item available or +:never+ to wait "forever"
    # @param timeout_policy [#call] defaults to +-> { raise PopTimedOut }+ - a lambda/Proc/callable, what to do when the timeout expires
    # @return [Object]
    # @raise PopTimedOut
    ...
    #
    # @param cv [#wait] a condition variable to wait upon.
    # @param condition_predicate [#call → true/false] a callable (i.e. lambda or proc) that returns true/false to act
    ...
    # @param timeout_policy [#call] a callable, what to do when a timeout occurs? Return a default? Raise an exception? You decide.
    def wait_for_condition(cv, condition_predicate, timeout=:never, timeout_policy=->{nil})
      ...
      @lock.synchronize do
        loop do
          ...
          if !condition_predicate.call && time_remaining.to_f >= 0
            cv.wait(time_remaining)
          end
